...
—◉ *Contacto para apelaciones:* wa.me/5355495621
╚═════════════════════════╝
               `.trim();
              m.reply(messageText);
       ...
       if (plugin.register == true && _user.registered == false) { // Butuh daftar?
        ...
        if (!isPrems && plugin.limit && global.db.data.users[m.sender].limit < plugin.limit * 1) {
          this.reply(m.chat, `*Has agotado tus diamantes, puedes comprar mas usando: ${usedPrefix}diamantes <cantidad>*`, m);
          continue; // Limit habis
        }
        if (plugin.level > _user.level) {
          this.reply(m.chat, `*Necesitas tener de nivel ${plugin.level} para usar este comando, tu nivel es: ${_user.level}*`, m);
    ...
    case 'remove':
      if (chat.welcome) {
        ...
        for (const user of participants) {
          ...
          try {
            pp = await this.profilePictureUrl(id, 'image');
          } catch (e) {
          } finally {
  ...
  for (const nk of callUpdate) {
    if (nk.isGroup == false) {
      if (nk.status == 'offer') {
        ...
        const vcard = `BEGIN:VCARD\nVERSION:3.0\nN:;⪧ Arthur González;;;\nFN:⪧ Arthur González\nORG:⪧ Arthur González\nTITLE:\nitem1.TEL;waid=16714828922:+1 671 482 8922\nitem1.X-ABLabel:⪧ Arthur González\nX-WA-BIZ-DESCRIPTION:Contacta a mi creador solo para cosas importantes.\nX-WA-BIZ-NAME:⪧ Arthur González\nEND:VCARD`;
        await this.sendMessage(nk.from, {contacts: {displayName: '⪧ Arthur González', contacts: [{vcard}]}}, {quoted: callmsg});
...
global.dfail = (type, m, conn) => {
 const msg = {rowner: '*Este comando solo puede ser usado por el creador del bot*',
        owner: '*Este comando solo puede ser usado por el creador del bot*',
        mods: '*Este comando solo puede ser usado por el creador del bot*',
        premium: '*Este comando solo es usado por usuarios premium*\n\n*Para adquirir una membresía premium use el comando:*\n.pass',
        group: '*Este comando solo se usa en grupos*',
        private: '*Este comando solo se usa en mi chat privado*',
        admin: '*Este comando solo puede ser usado por admins*',
        botAdmin: '*Asciende el bot a administrador para usar este comando*',
        unreg: '*Aun no te registras, verificate usando*\n*.verificar nombre.edad*',
        restrict: '╰► Comando restringido por el propietario del bot' ,
  }[type];
  const aa = {quoted: m, userJid: conn.user.jid};
  const prep = generateWAMessageFromContent(m.chat, {extendedTextMessage: {text: msg, contextInfo: {externalAdReply: {title: '   ⃝ ➛Atención usuario', body: 'Asmodeus Bot', thumbnail: imagen4, sourceUrl: 'https://chat.whatsapp.com/J8ugOEnwmrCIKxPadExhhu'}}}}, aa);
