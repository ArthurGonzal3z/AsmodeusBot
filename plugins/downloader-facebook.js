...
if (!args[0].match(/www.facebook.com|fb.watch/g)) throw `*Ingresa un enlace válido de Facebook, ejemplo: ${usedPrefix + command}* https://fb.watch/fOTpgn6UFQ/`
try {
await m.reply(`*Descargando video, por favor espere*`)      
const d2ata = await facebook.v1(args[0]);
let r2es = '';
if (d2ata.urls && d2ata.urls.length > 0) {
r2es = `${d2ata.urls[0]?.hd || d2ata.urls[1]?.sd || ''}`}
conn.sendFile(m.chat, r2es, 'error.mp4', `*Aquí tienes*`, m);
} catch (err1) {
try {  
...
conn.sendFile(m.chat, VIDEO, 'error.mp4', `*Aquí tienes*`, m)    
} catch (err2) {    
try {
let ress = await fg.fbdl(args[0])
let urll = await ress.data[0].url    
await conn.sendFile(m.chat, urll, 'error.mp4', '*Aquí tienes*', m)     
} catch (err3) {
try {
let res = await fbDownloader(args[0])
for (let result of res.download) {
let ur = result.url    
await conn.sendFile(m.chat, ur, 'error.mp4', '*Aquí tienes*', m)}
} catch (err4) {
try {
let res3 = await fetch(`https://latam-api.vercel.app/api/facebookdl?apikey=brunosobrino&q=${args[0]}`)  
let json = await res3.json()
let url3 = await json.video
await conn.sendFile(m.chat, url3, 'error.mp4', '*Aquí tienes*', m)         
} catch (err5) {    
try {
const { result } = await facebookdl(args[0]).catch(async _ => await facebookdlv2(args[0])).catch(async _ => await savefrom(args[0]))
for (const { url, isVideo } of result.reverse()) await conn.sendFile(m.chat, url, `facebook.${!isVideo ? 'bin' : 'mp4'}`, '*Aquí tienes*', m)    
} catch (err6) {
