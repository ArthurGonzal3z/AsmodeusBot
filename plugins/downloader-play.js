...
const handler = async (m, {conn, command, args, text, usedPrefix}) => {
  if (!text) throw `*Ingresa el nombre de una canción o video que desees buscar de YouTube, ejemplo:*\n*${usedPrefix + command} Good Feeling - Flo Rida*`;
  try {
    ...
    const texto1 = `⪧\t\t ⃝   *_Descargando ${additionalText} de ${yt_play[0].title}_* 
⪧\t\t ⃝   *Enlace: ${yt_play[0].url}*
⪧\t\t ⃝   *Duración: ${secondString(yt_play[0].duration.seconds)}*  
\t\t\t\t\t\t\t *Vistas 👁️ ${`${MilesNumber(yt_play[0].views)}`}* `.trim();
    ...
    if (size >= limit_a2) {  
    await conn.sendMessage(m.chat, {text: `*El audio es demasiado grande para ser enviado, puede descargarlo en la siguiente página: ${dl_url}*`}, {quoted: m});
    return;    
    }     
    ...
    const size = fileSizeInMB.toFixed(2);    
    if (size >= limit2) {  
    await conn.sendMessage(m.chat, {text: `*Su video es demasiado grande para ser enviado,puede descargarlo directamente en la siguiente página: ${dl_url}*`}, {quoted: m});
    return;    
    }     
