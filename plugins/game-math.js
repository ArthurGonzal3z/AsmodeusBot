...
let handler = async (m, { conn, args, usedPrefix, command }) => {
    if (cooldowns.has(m.chat)) {
      let time = cooldowns.get(m.chat);
      let now = Date.now();
      let diff = now - time;
      if (diff < 1800000) {
        let remaining = 1800000 - diff;
        let minutes = Math.floor(remaining / 60000);
        let seconds = (remaining % 60000) / 1000;
        return conn.reply(
          m.chat,
          `🕐 Espere ${minutes} minutos y ${seconds} segundos para volver a usar este comando`
        );
      } else {
