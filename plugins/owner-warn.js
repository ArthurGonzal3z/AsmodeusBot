...
if (conn.user.jid == mention) return m.reply('No')    
if (user.banned) throw 'El usuario a sido baneado'
    if ((user.warn * 1) < 3) {
        user.warn += 1
        m.reply('Fuiste advertido por mi creador')
        await m.reply('y ahora tienes *' + (user.warn + 1) + '* Advertencias. Has sido baneado por recibir 4 advertencias', mention)
