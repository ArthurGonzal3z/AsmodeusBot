...
let handler = async (m, { conn, text, usedPrefix, command }) => {
  ...
  if (!Reg.test(text)) throw `Cambio de registro invalido, ingrese un "|" en medio del nombre y la edad ${usedPrefix + command} SinNombre|99`
  let [_, name, splitter, age] = text.match(Reg)
  if (!name) throw 'El nombre no puede estar vacio, ten en cuenta que debes poner un "|" en medio del nombre y la edad ${usedPrefix + command} SinNombre|99'
  if (!age) throw 'La edad no puede estar vacía, ten en cuenta que debes poner un "|" en medio del nombre y la edad ${usedPrefix + command} SinNombre|99'
