...
let enlace2 = { contextInfo: { externalAdReply: { showAdAttribution: true, mediaUrl: yt, mediaType: 'VIDEO', description: '', title: wm, body: 'AsmodeusBot', thumbnailUrl: await(await fetch(global.img)).buffer(), sourceUrl: yt }}}
...
`.trim())
    ...
    footer = (command.toLowerCase() == 'buyy' ?
(`
...
`.trim())
    ...
    image = (command.toLowerCase() == 'buyy' ?)
...
*𝙂𝙖𝙣𝙖𝙣𝙘𝙞𝙖𝙨: ${(listItems[item][paymentMethod] * total)} ${global.rpgshop.emoticon(paymentMethod)}*
*𝘼𝙝𝙤𝙧𝙖 𝙩𝙞𝙚𝙣𝙚: ${user[paymentMethod]} ${global.rpgshopp.emoticon(paymentMethod)}*
`.trim(), imgr, fkontak, time, m)
}
user.lastmiming = new Date * 1  
}
handler.help = ['buy', 'sell'].map(v => v + ' [item] [count]')
handler.tags = ['rpg']
handler.command = /^(buyy|selll)$/i
