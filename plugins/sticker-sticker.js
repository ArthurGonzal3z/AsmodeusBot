...
let handler = async (m, { conn, args, usedPrefix, command }) => {
  ...
  try {
    ...
    if (/webp|image|video/g.test(mime)) {
      ...
      if (!img) throw `*Ocurrió un error, verifique que haya respondido a una imagen, video o un enlace de una imagen con terminación .jpg para realizar la conversión a sticker usando el comando:${usedPrefix + command}*`
      let out
      try {
      ...
      else return m.reply('*El enlace es inválido, corrobore que sea similar a: ${usedPrefix}s https://telegra.ph/file/0dc687c61410765e98de2.jpg*')
    }
  } catch (e) {
    console.error(e)
    if (!stiker) stiker = e
  } finally {
    if (stiker) conn.sendMessage(m.chat, {sticker: stiker, headerType: 4, contextInfo: {
externalAdReply: {
"title": linkgc2,
"body": linkgc2,
"mediaType": 1, 
...
"sourceUrl": nna
    ...
    else throw '*Ocurrió un error, verifique que haya respondido a una imagen, video o un enlace de una imagen con terminación .jpg para realizar la conversión a sticker*'
  }}
handler.help = ['sfull']
...
let handler = async (m, { conn, args, usedPrefix, command }) => {
  ...
  try {
    ...
    } else throw `*Ocurrió un error, verifique que haya respondido a una imagen, video o un enlace de una imagen con terminación .jpg para realizar la conversión a sticker usando:${usedPrefix + command}*`
  } catch (error) {
    console.log(error)
    try {
      let [packname, ...author] = args.join` `.split`|`
      ...
      if (args[0] && isUrl(args[0])) {
        ...
        throw `*Enlace inválido, corrobore que sea similar a: https://telegra.ph/file/0dc687c61410765e98de2.jpg*`
      }
    } catch (error) {
      stiker = `*Ocurrió un error, verifique que haya respondido a una imagen, video o un enlace de una imagen con terminación .jpg para realizar la conversión a sticker*`	
    }
  } finally {
    m.reply(stiker)
  }
